...
原样移植原型 `:root`，落到 `tms-web/src/styles/tokens.css`：
...
**禁止在页面里写死颜色、自造圆角与间距。** 需要新令牌时走第 8 章的公共改动流程。
...
25 个菜单页加 3 个非菜单界面。菜单键与详细设计 6.2.2 权限目录逐一对应。「原型入口」是原型里负责渲染该页的函数，复刻时从那里抽结构。
...
1. **抽原型**：从 `prototype/TMS-原型.html` 取该页的渲染函数与 `configs[菜单键]`，列出区块顺序、列与列序、筛选项、按钮文案与位置、空态文案。
...
5. **比对验收**：按第 9 章执行，并把第 4 章的复刻状态改为「已完成」。
...
| # | 差异 | 裁决 | 依据 |
|---|---|---|---|
| 1 | 原型 `PLATFORM_ONLY` 含 `grants`、`codes`，共 7 个平台专属菜单 | 以文档为准，平台专属只有 `mode`、`cas`、`servercerts`、`customers`、`products` 五个 | 详细设计 6.2.1 已记录该差异 |
| 2 | 原型对平台排除 RKI 三菜单（`keys`、`rki`、`rki-records`） | 以文档为准，内置平台管理员覆盖全部权限码 | 详细设计 3.2.5 第 14 条、7.3 |
| 3 | 原型 `badge()` 用中文正则决定状态颜色 | 改为按枚举编码映射，文案走 `enums.*` | 详细设计第 6 章；中文正则在英文界面失效 |
| 4 | 原型登录页 `.login-wrap` 是左右分栏（`.login-art` 品牌区 + `.login-box` 表单），无验证码 | 布局按原型改成分栏，**保留验证码行** | 详细设计 3.1.5 第 12 条 |
| 5 | 原型「忘记密码」在登录页内原地切换标题与表单，现有实现是弹窗 | 按原型改成原地切换，验证码行同样保留 | 原型 `showLogin(forgot)` |
| 6 | 原型全部中文硬编码 | 复刻时一律进 i18n，中英双份 | 详细设计 7.13 |
| 7 | 原型「修改密码」是弹窗（`changeMyPassword()`），详细设计把「首次改密页」列为独立页面 | 两者都要：个人中心里的主动改密按原型走弹窗；首次登录与管理员重置后的强制改密仍是独立页面，因为此时要拦截其余所有页面 | 详细设计 3.1.2、3.1.5 第 9 条 |
| 8 | 原型用 `me().readOnly`、`view==='platform'` 这类演示态控制按钮显隐 | 换成 `v-perm` + 真实权限码；平台／客户差异走 `platform_only`，代码中不得出现硬编码菜单名单 | CLAUDE.md 权限红线 |
| 9 | 原型顶栏有全局设备 SN 搜索，现有实现没有 | 按原型补上，随 `devices` 页（3.9）一起做 | 原型 `globalSearch()` |

## 7. 国际化

- 所有可见文字走 i18n，`tms-web/src/i18n/locales/zh-CN.ts` 与 `en-US.ts` **必须同时改，键一一对应**。
- 沿用现有分段命名，不另起一套：公共文案进 `common`，枚举进 `enums`，菜单名进 `menu`／`menuGroup`，页面文案进与该页对应的段（已有 `login`、`org`、`member`、`role`、`customer`、`product`、`operLog`、`loginLog`、`session`、`profile`、`home`）。新页面的段名用菜单键的驼峰形式，如 `otaRecords`、`serverCerts`。
