...
| 逻辑删除 | 主数据表用 `deleted TINYINT` 默认 0；日志、流水、记录类表不做逻辑删除 |
...
|---|---|---|---|
| `PRIMARY` | `id` | 主键 | — |
| `uk_name` | `name`, `deleted` | 唯一 | 客户名称唯一 |
...
| 索引名称 | 字段 | 类型 | 用途 |
|---|---|---|---|
| `uk_parent_name` | `parent_id`, `name`, `deleted` | 唯一 | 同级名称唯一 |
...
| 索引名称 | 字段 | 类型 | 用途 |
|---|---|---|---|
| `uk_account` | `account_lower`, `deleted` | 唯一 | 账号唯一（大小写不敏感） |
| `uk_email` | `email`, `deleted` | 唯一 | 邮箱唯一绑定，允许多个 NULL |
...
| 索引名称 | 字段 | 类型 | 用途 |
|---|---|---|---|
| `uk_org_name` | `owner_org_id`, `name`, `deleted` | 唯一 | 同机构角色名唯一 |
...
| 索引名称 | 字段 | 类型 | 用途 |
|---|---|---|---|
| `uk_name` | `name`, `deleted` | 唯一 | 产品名称唯一 |
...
| 索引名称 | 字段 | 类型 | 用途 |
|---|---|---|---|
| `uk_model` | `model`, `deleted` | 唯一 | 型号标识唯一 |
