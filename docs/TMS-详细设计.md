...
| 图形验证码 | easy-captcha 1.6.2 | 登录与找回密码的人机验证，字体随包内置，不依赖宿主机字体 |
...
| 页面 | 页面职责 | 主要操作 | 使用角色 |
|---|---|---|---|
| 登录页 | 账号密码加图形验证码登录，展示登录失败原因 | 登录、换一张验证码、找回密码 | 全部 |
...
```mermaid
flowchart TD
    A[提交账号密码与验证码] --> A1{验证码正确且未过期?}
    A1 -- 否 --> F0[记录登录失败, 返回验证码错误]
    A1 -- 是 --> B{账号存在且未删除?}
...
12. 登录与找回密码每次都要求图形验证码。验证码由 `GET /api/auth/captcha` 签发，答案只存 Redis，默认 2 分钟过期，比对忽略大小写。
13. 验证码校验排在账号查询与密码比对之前，且**失败不累加登录失败次数**——否则攻击者用错误验证码刷某账号即可将其锁定，等于新增一个拒绝服务入口。验证码失败同样写登录日志。
14. 验证码一次性消费：无论校验成败都立即删除答案，同一标识不能重复提交，前端在任何登录失败后必须重新取码。
15. 验证码不按会话入口豁免，`tool` 入口同样强制校验，避免改换入口取值绕过；激活工具需先调用取码接口。

#### 3.1.6 异常场景

| 异常场景 | 系统处理 |
|---|---|
| 验证码错误或已过期 | 返回验证码错误，不校验账号密码、不累加失败次数；记录登录失败 |
...
| Method | Path | 说明 | 权限 |
|---|---|---|---|
| GET | `/api/auth/captcha` | 签发图形验证码 | 无需认证 |
...
| `captchaId` | String | 是 | 取码接口返回的验证码标识 |
| `captchaCode` | String | 是 | 用户填写的验证码，比对忽略大小写 |
...
错误：`AUTH_001` 账号或密码错误、`AUTH_002` 账号已停用、`AUTH_003` 账号已锁定、`AUTH_008` 验证码错误或已过期。

#### 5.3.2 `GET /api/auth/captcha`

无请求参数。响应 `data` 含 `captchaId` 与 `image`，`image` 为 `data:image/png;base64,...` 形式的图片，前端直接用于 `img` 标签。答案只存 Redis，接口不回传；验证码一次性消费，每次登录或找回密码前都要重新取码。

`POST /api/auth/password/forgot` 的请求体同样必填 `captchaId` 与 `captchaCode`。

#### 5.3.3 `GET /api/auth/profile`
